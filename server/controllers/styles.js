...
export const getStyles = async (req, res, next) => {
    try {
      const styles = await Styles.find({}).sort({createdAt:-1});
  ...
  export const getStylesPerPage=async (req, res) => {
    try {
      let query = Styles.find().sort({createdAt:-1});
  
      const page = parseInt(req.query.page) || 1;
      const pageSize = parseInt(req.query.limit) || 5;
      const skip = (page - 1) * pageSize;
      const total = await Styles.countDocuments();
  ...
  export const getOneStyle=async(req,res,next)=>{
    ...
    try {
      ...
      const data = await Styles.findOne({ _id: new ObjectId(projectId) });
  ...
  export const updateStyle =async(req,res)=>{
    ...
    try {
      const result =  await Styles.findByIdAndUpdate(filter,
        {
          stylesImg: req.body.stylesImg, 
...
export const deleteStyle = async (req, res) => {
  try {
    ...
    const result = await Styles.deleteOne(filter);
